...
carries the project-specific rules; the general code-quality rules are global, in
`~/.claude/CLAUDE.md` (symlinked as `~/.codex/AGENTS.md`). The original proposal is kept below
for its rationale.
...
| 2026-09-22 | General code-quality, verification and git rules moved from `CLAUDE.md` to the global `~/.claude/CLAUDE.md`, with `~/.codex/AGENTS.md` symlinked to it, so every project gets them. `CLAUDE.md` keeps only this mod's edges, test targets and docs. | Instruction files only. Sibling mods still need their own fact audit, one repo per session. |
| 2026-09-22 | Step 3 of the M1 setup: JUnit 5 added (same BOM as skyblock-flipper), `installMod` now depends on `check`. First test pins the saved swatch spellings (`SwatchTest`, 4 tests). E1 partly addressed: a behavior suite exists and runs in `build` and CI. | `./gradlew test` passes 4/4. Changing `written()` to upper-case hex made one test fail, so the test detects format drift. With test sources but no tests, Gradle 9.5.1 fails `test` (`failOnNoDiscoveredTests`); deleting `src/test` entirely would still pass silently, which was judged not worth a custom guard. Next: the rest of M1 (`ConfigFile` atomic save, clamped load, legacy-fill fixtures). |
| 2026-09-22 | M1 code-complete. `ConfigFile` saves through a temporary file and an atomic move; loading requires each control's JSON type, rejects NaN/Infinity and clamps numbers to the control's limits; an old `boxes.fillOpacity` now also sets `beams.fillAlpha` to `beams.alpha * share` when the file lacks it. E2, E3 and E4 closed in code. Decision on E4's absent colour keys: retain, no change, because every old save wrote every key its version had. | `ConfigFileTest`, 11 tests; 15/15 pass. Each fix was removed in turn and one test failed each time. A copy of the player's live config loads and saves back identical. Not reproducible in a unit test: a crash in the middle of a write. Game checks: M1-01 new; S4-10, S6-03 OPEN; the live config no longer has `boxes.fillOpacity`, so S6-03 needs a backup of an older file. |
| 2026-09-22 | M1 done: player marked M1-01 PASS (changed other settings than the steps named; they saved and came back). Passed checks moved word for word to `in-game-checks-passed.md` (18 checks); `in-game-checks.md` keeps the 24 still to do, with rule 10 for future moves. | S4-10 and S6-03 need a config file from before per-colour fills; the live one is already migrated, so the player may mark them `SKIP` — `ConfigFileTest` covers both migrations. Superseded S5-02/03/04/05 could also be marked `SKIP` in favour of S6-01/02. Player text in S4-10 has a stray `and/` after `livid.hud.*`; left as is. Next: M2. |

**Concrete next task:** M2, isolated dungeon runs. `DungeonState.tick` treats a new `ClientLevel`
only as a reason to invalidate the shared locraw; it keeps the old sidebar, floor and boss state
and does not advance `generation`, so room, puzzle and mob state from the previous run can
survive a warp into a new run on the same floor. First write a test that feeds `DungeonState` a
level change without a null-level tick and asserts `generation` advances and old state clears;
that needs a small seam, because `tick` reads `Minecraft` directly. Keep Livid's separate
fight-reset semantics. Then fix, and add MOD-02 as the game check.
